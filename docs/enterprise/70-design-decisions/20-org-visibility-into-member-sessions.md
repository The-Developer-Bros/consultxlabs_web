...
## Addendum (2026-09-10) — #1300: the quality aggregate breaks down per expert, and the floor is the thing that makes it safe

The 2026-08-21 addendum settled that an organisation's CSAT visibility is the aggregate card and nothing else. This addendum extends _what the aggregate is grouped by_, and tightens the number underneath it.

An enterprise buyer's actual question is not "how satisfied are our people overall" — it is **"is this expert worth re-booking"**. One org-wide average cannot answer it, so an organisation with four providers could not tell a good one from a bad one even in aggregate, and the only route to the answer was a support ticket. `/feedback-summary` now returns a per-consultant breakdown alongside the org total. It remains aggregate: no row, no rating, no comment and no member identity leaves the endpoint, and there is still no drill-down. A consultant's _name_ crosses over, which it already does on the organisation's own appointments feed, and BetterUp's employer dashboard segments the same way.

What makes that safe is not the grouping, it is the arithmetic underneath it, and the previous numbers were not sufficient for it.

**The floor moves from 3 respondents to 5.** Three was chosen when `AppointmentFeedback` was one row per booking. Once feedback became one row per call (#1268), three _rows_ stopped being three _people_ — a single member rating three sessions of one subscription cleared a row-counted floor alone, and the "average" handed back was their own private rating. #1268 fixed the denominator to count distinct raters; this fixes the number. Five is Microsoft Viva Glint's published default for reporting a score, and Culture Amp reports the same. Ten is the corresponding floor for free-text, recorded in `lib/enterprise/quality-thresholds.ts` against the day somebody asks for aggregated comments; nothing reaches it today because no organisation surface returns a comment at all.

**The counts are suppressed too, not just the average.** This addendum's own stated reason for suppressing is that a count makes the disclosure trivial, and the endpoint then returned `totalResponses` and `respondents` unconditionally — so at one respondent an organisation learned that exactly one member had rated exactly one session. Below the floor the endpoint now returns nulls for the average _and_ both counts, and the card says "needs 5 people" rather than rendering a blank it cannot explain.

**Secondary suppression, because a floor alone does not survive a breakdown.** Publish per-consultant figures beside an org total and subtraction is the attack: if exactly one cohort is hidden, its value is the total minus the published ones. So the rule is not "hide the small ones", it is **never hide exactly one** — every cohort below the floor is hidden, and if that leaves precisely one hidden, the smallest surviving cohort is hidden with it. Either nothing is withheld or at least two are, and only their sum is derivable. Glint ships this as a second minimum on the number of suppressed groups; Qualtrics merges a too-small group into the next-smallest instead. `__tests__/enterprise/org-quality-cohorts.test.ts` pins it, including the case where an organisation has exactly one expert below the floor and the honest answer is to publish nothing.

**The thresholds are code constants and stay that way.** Glint freezes its thresholds once a survey has collected data, because an administrator who can lower the bar after seeing the shape of the responses can lower it until the aggregate identifies somebody. A per-organisation setting would hand the tuning dial to the party the floor protects.

**The grant is its own key.** The endpoint moves from `operations.read` to a new `quality.read`. Same roles today — OWNER, MAINTAINER, MANAGER, SUPPORT — so nothing changes about who can read it. The point is that `operations.read` is also what opens the org-wide appointments feed, the recordings list and the documents list, which are spend-and-utilisation surfaces; this one is aggregated satisfaction drawn from ratings this ADR classifies as content. Sharing one grant means the day somebody widens `operations.read` for an unrelated reason, they widen this too without noticing.

Two exclusions were also added to the aggregate's `where`, both from #1300: a moderation-removed rating leaves with its comment, and a rating adjudicated as caused by a platform failure is excluded — an outage in our video stack must not read to an organisation as a bad consultant any more than it should count against the consultant's public score.
