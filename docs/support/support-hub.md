...
| Org triage — threads             | `app/dashboard/organization/[orgId]/support/OrgSupportTriage.tsx` + `GET /api/organizations/[orgId]/support-threads` | `operations.read` → **metadata-only** thread list, no bodies (ADR 20)                                                                                                                                                                                                                                                                                          |
| Org triage — quality             | the same page + `GET /api/organizations/[orgId]/feedback-summary`                                                    | `quality.read` → aggregates only, per consultant, floored and secondarily suppressed. Its own grant rather than `operations.read`, which is also what opens the appointments feed, the recordings list and the documents list ([ADR 20](../enterprise/70-design-decisions/20-org-visibility-into-member-sessions.md), 2026-09-10 addendum)                     |
