...
Each row records `supersededAt`, when this text stopped being the live text, and `editorUserId`, who caused the supersession. Normally that is the author, but occasionally it is a staff redaction: `isPrivileged` admits STAFF and ADMIN into the `PUT`, and before the trail existed a staff member could rewrite the text of a consumer review and the row afterwards was indistinguishable from an author edit, with no `ModerationAction`, no audit row and no attribution. Under FTC 16 CFR §465 that is the highest-exposure write in the subsystem. A surface that publishes "edited" must be able to tell the two apart, so the trail runs on the staff path too and records who did it. The editor relation is `SetNull`, because a departed staff member must not delete a consumer's edit history.
