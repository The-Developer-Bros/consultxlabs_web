...
`softDeleteReview` in `lib/moderation/side-effects.ts` runs inside the action transaction. It stamps `deletedAt` and `removedBy = MODERATION` in one `updateMany` CAS'd in the `WHERE`, and the report's `ModerationAction` row carries `reviewId` so the actor and reason are one join away (#1562): it lands on a live row or one the author had withdrawn (moderation wins over a withdrawal, so the author cannot revive content staff removed), and it is a no-op on a row moderation had already taken down, so the second of two concurrent removals writes nothing rather than overwriting the first one's timestamp and attribution. It then recomputes the consultant's scores in the same transaction, and the action route purges the public caches afterwards, so a removed review stops rendering on the landing page and explore within the request rather than within the hour. The purge at `app/api/staff/moderation/reports/[reportId]/action/route.ts` is present; the audit claim that `CONTENT_REMOVED` did not purge was checked and is stale.

The ADMIN-only `DELETE /api/staff/moderation/reviews/[reviewId]` is the other staff removal path, and it now mirrors `softDeleteReview`'s ordering rule: it stamps `deletedAt` and `removedBy = MODERATION`, CAS'd in the `WHERE` so a takedown lands even on a row the author had withdrawn and is a no-op on a row moderation had already removed, and writes one `ModerationAction { REVIEW_REMOVED, reviewId, takenById }` in the same transaction — before #1562 this direct takedown wrote no audit row at all. A staff takedown of a reply through `DELETE /api/user/reviews/[id]/reply` writes `REVIEW_REPLY_REMOVED` the same way; the consultant's own withdrawal stamps `replyRemovedBy = AUTHOR` and no audit row, because it is not a staff act.
