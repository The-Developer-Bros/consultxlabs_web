...
[ADR 29](../enterprise/70-design-decisions/29-two-track-reputation-and-the-right-of-reply.md) specifies `@@unique([consultantProfileId, consulteeProfileId, track])`, so a client who attends a webinar and later books the same consultant one-to-one holds one review of each product rather than one that overwrites the other. The key on this branch is the two-column one, and the reason is sequencing, not doubt.
...
2. That leaves a window in which the new schema is live and the old code is still serving, so every statement has to be backward compatible with what is deployed. Replacing a unique is the one statement that is not. Prisma derives its compound-key name from the columns, so `consultantProfileId_consulteeProfileId` becomes `consultantProfileId_consulteeProfileId_track`, and `dev`'s deployed `app/api/user/reviews/route.ts` looked the old name up by that compound key. Pushing the wider key would have broken review creation in production until this branch deployed.

Keeping two columns makes the whole plan additive, so its push is safe to run ahead of its own deploy, and `npm run db:preflight` reports it as additive with an empty allowlist. Nothing on this branch references the compound key any more: the write path is `findFirst` on the pair plus `update({ where: { id } })`, and the composer's lookup is keyed by consultant alone. So the DDL for #1549 is deployable on its own: replace the unique and add the one allowlist entry for the drop. Two code changes land in the same PR as that DDL, because each is wrong under the other key. The `POST /api/user/reviews` lookup must filter on `reviewable.track`, with a fallback to a `NULL`-track legacy row that the write then adopts; under today's pair-only key that filter would miss a GROUP row while writing a one-to-one review, fall through to `create`, and hand the author a `P2002` for a row the form never showed them, which is why the lookup is deliberately pair-only on this branch. And `reviewsByConsultant` in `lib/reviews.ts` must key by `(consultant, track)` so the composer can show a client both of their reviews of one person. The create cannot fail on the data, because there were 0 duplicate pairs across 62 rows and adding a column to a unique key can only split groups, never merge them. Until it lands, a mixed-mode client holds one review rather than two, filed under whichever product they reviewed first, which is `dev`'s existing behaviour plus a track label.
