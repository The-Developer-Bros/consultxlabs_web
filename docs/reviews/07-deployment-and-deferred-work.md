...
Keeping two columns made the review-key part of the plan additive, so the push was safe to run ahead of its own deploy; the one statement `npm run db:preflight` gated was the foreign-key swap on `ConsultantReviewRevision` described below, which was allowlisted for exactly that reason and has since been applied. Nothing on this branch references the compound key anymore: the write path is `findFirst` on the pair plus `update({ where: { id } })`, and the composer's lookup is keyed by consultant alone. That makes #1549's DDL backward-compatible with the deployed code, which is a statement about safety and not about sufficiency. The DDL must not ship alone: under the wider key, pair-only code still finds the other track's row on a mixed-track submission and rewrites it, and a repeat `GROUP` submission still moves the one row to the latest `ratingUnitId` instead of creating an event review. The sidecar index, the `POST /api/user/reviews` lookup and the `reviewsByConsultant` re-key are one rollout, as the next paragraph sets out.
