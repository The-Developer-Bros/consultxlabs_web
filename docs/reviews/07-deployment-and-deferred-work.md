...
The order was: the #1542 release to `prod` first (so no running code referenced the old compound key), then the PR green on its preview (nothing prerendered selects a new column, so the build passes before the push), then the push immediately before the merge, then the merge and the `dev` → `prod` release straight after. The push itself was wrapped in the existing maintenance gate and ran as six steps rather than `npm run db:push`, because the two removal-pair CHECKs cannot be added while a removed row has no actor and the live database held three such rows from the 2026-09-11 E2E pass: (1) `POST /api/admin/maintenance { phase: "OFFLINE" }` on production, so no review write could land between the checks below and the deploy; (2) part 1 of `prisma/sql/one-off/2026-09-12-stamp-review-removal-actor.sql`, the SELECT that captures the author's own withdrawals while `deletedByUserId` still exists; (3) `npm run db:preflight`, then `npx prisma db push` (the `db:push:no-sidecars-DANGEROUS` script); (4) part 2 of the one-off, which stamps `AUTHOR` for the captured ids and `MODERATION` for every other removed row; (5) `npm run db:sidecars` and `npm run db:assert-sidecars`; (6) merge, deploy `dev` and the `dev` → `prod` release, smoke-check, then `DELETE /api/admin/maintenance`. The thirteen allowlist entries were removed from `known-drift.json` in a commit on the branch before the merge, not after: `db:preflight` matches them exactly until their expiry, so a checked-in allowlist would let another worktree repeat the destructive push. A database built from scratch has no removed rows and needs no one-off.

If any step after the push fails — the stamp, a sidecar, the assertion, a deploy or a smoke check — the site stays OFFLINE: the gate's 24-hour TTL bounds the outage, it does not recover from it. There is no rollback of a destructive push; the operator repairs forward (the sidecars are idempotent, the one-off is idempotent, `db:assert-sidecars` re-checks), re-runs the assertions and the smoke checks, and only lifts maintenance once the deployed build answers `/api/health` and completes one review write. The general post-maintenance recovery runbook ends maintenance before its checks and does not cover a half-completed destructive cutover; this paragraph does. Two tooling gaps this cutover exposed are filed rather than fixed here: `db:preflight` and `prisma db push` are two processes with no lock between them, so another worktree can change the database in the gap, and a bare `npx prisma db push` bypasses the guard entirely. Between the push and the deploys, the running code's review writes and the staff review queue fail on the dropped columns — a window of minutes, accepted pre-MVP over an expand-and-contract pair of pushes that would have doubled the prod operations for the same result.
