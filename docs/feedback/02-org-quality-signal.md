...
**Never hide exactly one cohort's worth of people.** A floor does not survive a breakdown published beside a total: hide exactly one consultant and its value is the total minus the published ones, so an administrator with a calculator recovers precisely the thing the floor exists to hide. Cohorts carry `respondentIds`, not row counts, because one person can sit in several cohorts and "two hidden groups" can be two ratings from the same person; the guard has to clear the floor in **distinct people**, not in cohort count. `applyCohortSuppression` hides every cohort below the floor and keeps pulling in the smallest surviving cohort until the hidden set's distinct people clear `ORG_QUALITY_MIN_RESPONDENTS`, or, failing that, hides everything. The count of hidden cohorts, `consultantsSuppressed`, is safe to state only once the hidden people themselves clear the floor — never "an administrator can count who's missing", and never a bare "at least two" as a substitute for actually checking. Below that, `suppressed` is 0, which is the difference between "we have no data on them" and "we are not telling you": a count is stated only when the hidden people themselves clear the floor, not "never one, by construction" as a blanket rule.

A row whose slot carries no `consultantProfileId` feeds the same arithmetic as an UNATTRIBUTED pseudo-cohort, keyed on an empty string rather than a real consultant id, so those responses still count toward the total and toward suppression decisions without ever appearing as a breakdown line of their own. `consultantsSuppressed` counts named cohorts only — the UNATTRIBUTED bucket is excluded from both the published and the hidden count the endpoint reports — because "one expert withheld" would otherwise sometimes describe a bucket that names nobody.
