...
import Slider from "react-slick"; // install this package
...
const UserCarousel = () => {
  ...
  useEffect(() => {
    axios.get('https://retoolapi.dev/f0fmcj/data')
      .then(response => {
  ...
  return (
    <Slider {...settings}>
      {users.map(user => (
        <div key={user.id} style={{ borderRadius: '10px', width: '30px', height: '30px', position: 'relative' }}>
          {/* Replace with your actual image */}
          <Image src="/path-to-your-image.jpg" alt={user.Name} layout="fill" objectFit="cover" /> {/* Use next/image's Image component here */}
          <p>{user.Name}</p>
