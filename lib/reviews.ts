 ...
 * the honest choice is the track that matches what the surface is selling: a
 * person card prefers their 1:1 reputation, a webinar or class card prefers the
 * group one. Falling back to the other track is deliberate — a consultant who
 * only ever runs webinars has a real, earned score and hiding it because the card
 * happens to be a person card would be worse than labelling it.
 *
 * NULL means SUPPRESSED, and every caller must render that as "not enough rated
 * sessions yet" rather than as 0.0. This is the whole reason the raw mean is no
